...
import { Meal, Role, DietaryType } from '@/types';
...
export default function MealCard({ meal }: MealCardProps) {
  ...
  // Show add to cart button only for customers or when not logged in (will redirect to login)
  const showAddToCart = !isAuthenticated || user?.role === Role.CUSTOMER;

  const getDietaryBadge = () => {
    // Return early if dietaryInfo is undefined or NONE
    if (!meal.dietaryInfo || meal.dietaryInfo === DietaryType.NONE) {
      return null;
    }
    ...
    > = {
      [DietaryType.NONE]: { label: '', variant: 'info' },
      [DietaryType.VEGETARIAN]: { label: '🥬 Vegetarian', variant: 'success' },
      [DietaryType.VEGAN]: { label: '🌱 Vegan', variant: 'success' },
      [DietaryType.GLUTEN_FREE]: { label: '🌾 Gluten-Free', variant: 'info' },
      [DietaryType.DAIRY_FREE]: { label: '🥛 Dairy-Free', variant: 'info' },
      [DietaryType.NUT_FREE]: { label: '🥜 Nut-Free', variant: 'warning' },
      [DietaryType.HALAL]: { label: '☪️ Halal', variant: 'info' },
      [DietaryType.KOSHER]: { label: '✡️ Kosher', variant: 'info' },
    };
    ...
    <Link href={`/meals/${meal.id}`}>
      <div className="card group hover:shadow-lg transition-shadow duration-300">
        ...
        <div className="relative h-48 bg-gray-200">
          ...
          {dietaryBadge && dietaryBadge.label && (
